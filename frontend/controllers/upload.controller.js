angular.module("hireSenseApp").controller("UploadController", ["$scope", "$rootScope", "$timeout",
    function ($scope, $rootScope, $timeout) {
        ...
        $scope.handleFileInput = function (inputEl) {
        ...
        $scope.onDrop = function (event) {
            event.preventDefault();
            $scope.dragOver = false;
            var files = Array.prototype.slice.call((event.dataTransfer && event.dataTransfer.files) ? event.dataTransfer.files : []);
            $scope._addFiles(files);
            if (!$scope.$$phase) {
                $scope.$apply();
            }
        ...
        // ── Add files to queue ─────────────────────────────────────────────────
        $scope._addFiles = function (files) {
            var allowed = ["pdf", "txt", "doc"];
            files.forEach(function (f) {
                var ext = f.name.split(".").pop().toLowerCase();
                if (allowed.indexOf(ext) !== -1) {
                    var exists = $scope.uploadQueue.some(function (q) {
                        return q.name === f.name && q.size === f.size;
                    });
                    if (!exists) {
                        $scope.uploadQueue.push(f);
                    }
                }
            });
        };
